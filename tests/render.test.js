...
test('renderIdentityLine includes speed when showSpeed is true and speed is available', async () => {
...
test('renderIdentityLine omits speed when showSpeed is false', async () => {
  await withDeterministicSpeedCache(async ({ transcriptPath }) => {
    const ctx = baseContext();
    ctx.stdin.transcript_path = transcriptPath;
...
test('render expanded layout includes speed on identity line and duration on project line', async () => {
  await withDeterministicSpeedCache(async ({ transcriptPath }) => {
    const ctx = baseContext();
    ctx.stdin.transcript_path = transcriptPath;
    ctx.config.lineLayout = 'expanded';
    ...
    // Speed is now on identity line (context bar line), not project line
    const identityLine = lines.find(line => line.includes('Context'));
    const projectLine = lines.find(line => line.includes('my-project'));

    assert.ok(identityLine, 'expected an identity line');
    assert.ok(identityLine.includes('out: 1000.0 tok/s'), 'should include speed on identity line');
    assert.ok(projectLine, 'expected a project line');
    assert.ok(projectLine.includes('⏱️  12m 34s'), 'should include session duration on project line');
  });
});
