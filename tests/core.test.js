...
// Context window mismatch tests
test('getBufferedPercent falls back to manual calculation when context window mismatch detected', () => {
  // DeepSeek reports 200K but actual model is 1M
  // Native percentage is calculated against wrong window
  // Should fall back to manual calculation with inferred 1M
  const percent = getBufferedPercent({
    model: { id: 'DeepSeek-V4-Flash', display_name: 'deepseek-v4-flash' },
    context_window: {
      context_window_size: 200000,
      current_usage: { input_tokens: 89000 },
      used_percentage: 45, // Claude Code calculated 45% against 200K
    },
  });
  // Manual calc: 89k/1M = 9%, with buffer ≈ 10%
  assert.ok(percent < 20, `expected percent < 20 (manual calc), got ${percent}`);
  assert.notEqual(percent, 45, 'should not use native percentage with mismatch');
});

test('getBufferedPercent uses native when context windows match (Claude 200K)', () => {
  // Claude with 200K - native and inferred match
  const percent = getBufferedPercent({
  ...
  assert.equal(percent, 47, 'should use native percentage when windows match');
});
