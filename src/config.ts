...
export interface HudConfig {
  ...
  display: {
    ...
    safeMode: boolean;
    // Show background tasks count (OMC-style: bg:X/Y)
    showBackgroundTasks: boolean;
    // Total background task slots (default 5)
    backgroundTaskSlots: number;
    // Color theme name (from COLOR_THEMES), overrides colors if set
    theme: string;
  };
...
function validateContextValue(value: unknown): value is ContextValueMode {
  return value === 'percent' || value === 'tokens' || value === 'remaining' || value === 'both' || value === 'percentTokens';
}
...
export function mergeConfig(userConfig: Partial<HudConfig>): HudConfig {
  ...
  const display = {
    ...
    safeMode: typeof migrated.display?.safeMode === 'boolean'
      ? migrated.display.safeMode
      : DEFAULT_CONFIG.display.safeMode,
    showBackgroundTasks: typeof migrated.display?.showBackgroundTasks === 'boolean'
      ? migrated.display.showBackgroundTasks
      : DEFAULT_CONFIG.display.showBackgroundTasks,
    backgroundTaskSlots: validateNonNegativeInteger(
      migrated.display?.backgroundTaskSlots,
      DEFAULT_CONFIG.display.backgroundTaskSlots,
    ),
    theme: typeof migrated.display?.theme === 'string' && migrated.display.theme.length > 0
      ? migrated.display.theme
      : DEFAULT_CONFIG.display.theme,
  };

  // Keep default colors so HudColorOverrides type is satisfied
  const colors = {
