 ...
 * s = Haiku (fast)
 */
function getAgentTierCode(agent: AgentEntry): string {
  const model = agent.model?.toLowerCase() ?? '';
  if (model.includes('opus')) return 'O';
  if (model.includes('sonnet')) return 'e';
  if (model.includes('haiku')) return 's';
  // Fallback based on agent type hints
  const type = agent.type.toLowerCase();
  if (type.includes('opus') || type.includes('architect') || type.includes('planner')) return 'O';
  if (type.includes('sonnet')) return 'e';
  if (type.includes('haiku')) return 's';
  return 'a'; // default agent
}

function getTierColor(code: string): (text: string) => string {
  switch (code) {
