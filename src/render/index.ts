...
import { renderBackgroundTasksLine } from './background-tasks-line.js';
import {
  renderIdentityLine,
  renderProjectLine,
  renderAddedDirsLine,
...
import { getDisplayDensity, isElementVisible, getTheme } from '../constants.js';
...
function renderExpanded(ctx: RenderContext, terminalWidth: number | null = null): Array<{ line: string; isActivity: boolean }> {
  ...
  const density = getDisplayDensity(terminalWidth ?? UNKNOWN_TERMINAL_WIDTH);
  ...
  for (let index = 0; index < elementOrder.length; index += 1) {
    ...
    // Check display density - skip elements not visible at current density
    if (!isElementVisible(element, density)) {
      continue;
    }
...
export function render(ctx: RenderContext): void {
  ...
  const safeMode = ctx.config?.display?.safeMode ?? true; // Default to safe mode for Windows compatibility
  ...
  // Apply theme colors if a custom theme is set
  const themeName = ctx.config?.display?.theme;
  if (themeName && themeName !== 'default' && ctx.config?.colors) {
    const theme = getTheme(themeName);
    // Completely replace colors with theme colors
    ctx.config.colors = { ...theme.colors };
  }
  ...
  if (lineLayout === 'expanded') {
    ...
    // Background tasks line (OMC-style, only when OMC is installed)
    if (ctx.config?.display?.showBackgroundTasks !== false && ctx.isOmc) {
      const bgTasksLine = renderBackgroundTasksLine(ctx);
      if (bgTasksLine) {
        lines.push(bgTasksLine);
      }
    }
  ...
  for (const line of visibleLines) {
    let outputLine = `${RESET}${line}`;
    // Safe mode: strip ANSI codes for Windows/terminal multiplexer compatibility
    if (safeMode) {
